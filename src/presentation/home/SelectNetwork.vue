<template>
    <div class="h-12">
        <div class=" ml-2.5 absolute bg-white">
            ...
            <Icon icon="cryptocurrency:eth" width="25" class="mr-1"  v-if="currentNetwork.name == 'Goerli'" />
            <Icon icon="cryptocurrency:bnb" width="25" class="mr-1"  v-if="currentNetwork.name == 'Binance-testnet'" />
            {{currentNetwork.name}} 
                    ...
                    <li v-for="(network,index) in avalaibleNetworksArray" :key="index">
                        <button @click="changeNetwork(network)" type="button" class="inline-flex bg-white py-2 px-4 w-full text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-white">
                            <div class="inline-flex items-center">
                                <Icon icon="cryptocurrency:eth" width="25" class="mr-1"  v-if="network.name == 'Goerli'" />
                                <Icon icon="cryptocurrency:bnb" width="25" class="mr-1"  v-if="network.name == 'Binance-testnet'" />
                                {{network.name}}
                            </div>
                        </button>
                    </li>
...
const avalaibleNetworksArray = ref([])
const collapseSelect = ref(false)
const emits = defineEmits(['changeSelectedNetwork'])
onMounted(() => {
    for(const network in avalaibleNetworks){
        avalaibleNetworksArray.value.push(avalaibleNetworks[network])
    }
    if(avalaibleNetworks[userStore.currentUserNetworkId]) {
        changeNetwork(avalaibleNetworks[userStore.currentUserNetworkId])
    } else {
        // PER DEFAULT GOERLI NETWORK
        changeNetwork(avalaibleNetworks["5"])
    }
})
...
function changeNetwork( newNetwork ){
    ...
    avalaibleNetworksArray.value = avalaibleNetworksArray.value.filter(value => value.id !== newNetwork.id)  
